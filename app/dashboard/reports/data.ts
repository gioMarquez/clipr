...
export async function getReportsData() {
	...
	const citasPorEstado = [
		{ name: "Completadas", value: statusCount.COMPLETED, color: "#22c55e" },
		{ name: "Confirmadas", value: statusCount.CONFIRMED, color: "#3b82f6" },
		{ name: "Pendientes", value: statusCount.PENDING, color: "#f59e0b" },
		{ name: "Canceladas", value: statusCount.CANCELLED, color: "#ef4444" },
	];

	// — Rendimiento por barbero —
	const barberMap = new Map<
		string,
		{ name: string; citas: number; ingresos: number }
	>();
	...
	const clientMap = new Map<
		string,
		{ name: string; citas: number; gasto: number }
	>();
	appointments.forEach((a) => {
